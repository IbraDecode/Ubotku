...
async def chat_gpt(client, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            prs = await message.reply_text(f"<emoji id=6226405134004389590>🔍</emoji> Processing your request...")
...
async def chat_gpt(client, message):
    try:
        ...
        if len(message.command) < 2:
            await message.reply_text(
                "<blockquote><b><emoji id=5019523782004441717>❌</emoji>mohon gunakan format\ncontoh : /ai halo</b></blockquote>"
            )
        else:
            prs = await message.reply_text(f"<blockquote><b><emoji id=6226405134004389590>🔍</emoji> Processing your request...</b></blockquote>")
            a = message.text.split(' ', 1)[1]
            response = requests.get(f'https://api.botcahx.eu.org/api/search/openai-chat?text={a}&apikey=Biyy')

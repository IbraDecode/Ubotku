...
uglifyjs_available = shutil.which("uglifyjs") is not None

@PY.UBOT("enc")
@PY.TOP_CMD
async def encrypt_js(client: Client, message: Message):
    if not uglifyjs_available:
        return await message.reply("<emoji id=5019523782004441717>❌</emoji> UglifyJS belum terinstal!\\n\\nInstall dengan: <code>npm install -g uglify-js</code>")
